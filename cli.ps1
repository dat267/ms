...
function Invoke-Cli {
  ...
  if (!$MainPayload -or $MainPayload -isnot [scriptblock]) {
    Write-Host "Define `$MainPayload = { param(...) ... } first" -ForegroundColor Red
    $global:LASTEXITCODE = 1; return 1
  }
  ...
  function Stop-Cli {
    ...
    if ($Host.Name -match 'ConsoleHost|ServerRemoteHost' -and $MyInvocation.CommandOrigin -ne 'Internal') {
      exit $ExitCode
    } else {
      return $ExitCode
    }
